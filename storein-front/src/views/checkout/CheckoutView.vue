<template>
  <div class="container-main py-8 min-h-[70vh]">
    ...
    <div v-if="!cartStore.items.length && !placing"
      ...
      <RouterLink :to="{ name: 'products' }" class="btn-brand px-6 py-2.5 text-sm">
        مشاهده محصولات
      </RouterLink>
    </div>
          ...
          <div class="rounded-2xl border border-surface-border p-5 space-y-4"
            ...
            <Transition name="expand">
              <div v-if="showAddressForm" class="border-t border-surface-border pt-4 space-y-4">
                <h3 class="text-sm font-bold text-text-primary">آدرس جدید</h3>

                <div class="grid grid-cols-2 gap-3">
                  <div class="flex flex-col gap-1">
                    <label class="form-label">عنوان <span class="text-error">*</span></label>
                    <input v-model="addrForm.title" class="form-input" placeholder="خانه، محل کار..." maxlength="30" />
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div class="flex flex-col gap-1">
                    ...
                    <input v-model="addrForm.province" class="form-input" placeholder="تهران" />
                  </div>
                  <div class="flex flex-col gap-1">
                    <label class="form-label">شهر <span class="text-error">*</span></label>
                    <input v-model="addrForm.city" class="form-input" placeholder="تهران" />
                  </div>
                </div>
                ...
                <div class="flex flex-col gap-1">
                  <label class="form-label">جزئیات (پلاک، طبقه، واحد) <span class="text-error">*</span></label>
                  <textarea v-model="addrForm.detail" class="form-input resize-none" rows="2" placeholder="پلاک ۱۲، طبقه ۳، واحد ۵" />
                </div>
          ...
          <div class="rounded-2xl border border-surface-border p-5 space-y-4"
            ...
            <div class="space-y-2">
              ...
              <textarea
                ...
                rows="2"
                placeholder="اگر توضیح خاصی برای سفارش دارید بنویسید..."
                maxlength="300"
              />
              ...
              <label :class="[
                ...
                <div>
                  ...
                  <p class="text-text-secondary text-xs mt-0.5">
                    {{ walletBalance < cartStore.totalPrice
                        ? `موجودی کافی نیست (کمبود: ${formatPrice(cartStore.totalPrice - walletBalance)})`
                        : 'پرداخت فوری بدون نیاز به درگاه' }}
                  </p>
                </div>
                  ...
                  <Transition name="expand">
                    <div v-if="paymentMethod === 'mixed'" class="mt-3 space-y-2">
                      ...
                      <p class="text-xs text-text-secondary font-fanum">
                        مابقی از درگاه: {{ formatPrice(cartStore.totalPrice - walletAmount) }}
                      </p>
                    </div>
                  </Transition>
                </div>
      ...
      <div class="rounded-2xl border border-surface-border p-5 flex flex-col gap-4 lg:sticky lg:top-24"
        ...
        <div class="space-y-2 text-sm border-t border-surface-border pt-3">
          ...
          <div v-if="couponApplied" class="flex justify-between text-success text-xs">
            ...
            <span class="font-fanum">اعمال شد ✓</span>
          </div>
          <div class="flex justify-between text-text-secondary">
            <span>هزینه ارسال</span>
            <span class="text-success font-medium">رایگان</span>
          </div>
        </div>
...
function validateAddrForm() {
  ...
  if (!/^0?9\d{9}$/.test(addrForm.value.recipientPhone))
    e.recipientPhone = 'شماره معتبر نیست'
  if (!/^\d{10}$/.test(addrForm.value.postalCode))
    e.postalCode = 'باید ۱۰ رقم باشد'
  addrErrors.value = e
